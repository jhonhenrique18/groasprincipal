...
| 2026-05-03 03:25 | Claude | commit | f8b0db5 feat(seo): expand guias para 26 + UX polish + Ver producto navigation (12 arquivos, +2780/-139) | f8b0db5 |
| 2026-05-03 03:26 | Claude | push | push 1e2f511..f8b0db5 origin/main com 3 commits SEO (4b18de9 Fase A + 8e0ab59 Fase D + f8b0db5 Fase D extended); Railway auto-deploy on main | origin/main |
| 2026-05-03 04:00 | Claude | write | Phase C: cria categories_data.py com 9 categorias (intros 250-400 palavras + FAQ 5 questões cada). Helper default_product_faq + product_faq_and_howto em app.py. FAQPage + HowTo schema em todas as 112 /producto/<slug>; FAQPage + intro em 9 /productos/<slug> | categories_data.py, app.py, templates/, static/css/style.css |
| 2026-05-03 04:05 | Claude | test | smoke test Phase C: 9/9 categorias renderizam intro+FAQ; /producto com guia tem 7 schemas (FAQ+HowTo do guide); /producto sem guia tem 6 schemas (FAQ default B2B) | http://localhost:5050 |
| 2026-05-03 04:10 | Claude | commit | d05ceca feat(seo-C): FAQPage+HowTo schema em produtos + 9 category hubs editoriais (7 files, +666/-2) | d05ceca |
| 2026-05-03 04:11 | Claude | push | push f8b0db5..d05ceca origin/main; Railway auto-deploy Phase C (categories + product FAQs) | origin/main |
| 2026-05-11 21:00 | Claude | read | lê AGENT_RULES/STATE/CONTEXT/PROJECT + checkpoints recentes; mapa via Explore agent das refs Grãos S.A. em templates/app.py/config.py/models.py | .planning/, templates/, app.py, config.py, models.py |
| 2026-05-11 21:05 | Claude | decision | convoca conselho de 4 vozes paralelas (Arquiteto/SEO Specialist/Cético/Compliance) para debate da remoção de Grãos S.A. das pesquisas; tensão nomeada: "sem rastros" vs "preservar autoridade SEO" antagônicos | conversa |
| 2026-05-11 21:30 | Claude | decision | usuário autoriza Fase 1 (cleanup total) + Fase 2 (modo deindex com 410 Gone) num único commit; fase de transição (manter graos visível) explicitamente revogada pelo usuário | conversa |
| 2026-05-11 21:32 | Claude | write | cria templates/maintenance.html neutro (sem branding, sem Especias del Paraguay no body — não vincular nova marca ao histórico durante quarentena) | templates/maintenance.html |
| 2026-05-11 21:34 | Claude | write | app.py: MAINTENANCE_MODE flag + before_request gate (410 Gone) + X-Robots-Tag global noindex+nofollow+noarchive+nosnippet + robots.txt Disallow: / + sitemaps 410 + errorhandler(404) vira 410 | app.py |
| 2026-05-11 21:36 | Claude | write | bulk cleanup templates (sed): Grãos S.A./SA/Paraguay → Especias del Paraguay em 10 templates; strip de https://www.graos.com.py em URLs JSON-LD; @id Organization legado → /#especias-organization (corte de continuidade) | templates/*.html |
| 2026-05-11 21:37 | Claude | write | config.py SQLite fallback graos.db → epy.db; models.py comentário limpo; .gitignore lp-graos-foz → lp-legacy-foz; app.py base URL hardcoded sitemap | config.py, models.py, .gitignore, app.py |
| 2026-05-11 21:38 | Claude | write | remove assets legacy: graos flor.png (raiz 691KB) + static/img/logo-verde.png (era referenciado no JSON-LD legado) | rm graos flor.png static/img/logo-verde.png |
| 2026-05-11 21:39 | Claude | test | smoke test test_client: rotas públicas 410+maintenance HTML, has_graos=False em todos bodies, /robots.txt 200 Disallow: /, /admin/login 200, X-Robots-Tag em todas respostas | http://localhost (test_client) |
| 2026-05-11 21:42 | Claude | write | checkpoint Fase 1+2 com posições do conselho, decisões tomadas, caminhos rejeitados, pendências para reativação | .planning/checkpoints/2026-05-11-graos-wipe-and-deindex.md |
| 2026-05-11 21:44 | Claude | write | seed.py + app.py: SiteSetting defaults (whatsapp/email/hero_image) trocados para empty string + init_db() agora wipea SiteSettings de contato no boot quando MAINTENANCE_MODE=1 (idempotente) — pedido do usuário "tire meu contato meu email deixa sem nada" | seed.py, app.py |
| 2026-05-11 21:47 | Claude | push | d85d5c1 origin/main (após git gc para resolver pack-objects signal 10); Railway auto-deploy on push | origin/main |
| 2026-05-11 21:48 | Claude | test | validação produção: GET https://www.graos.com.py/ → 410; /productos /producto/* /guias /nosotros /contacto /sitemap.xml → 410; /robots.txt → "User-agent: * / Disallow: /"; /admin/login → 200; X-Robots-Tag: noindex,nofollow,noarchive,nosnippet em todas respostas | https://www.graos.com.py |
| 2026-05-11 21:50 | Claude | write | segunda passada: remove refs legacy a logo-verde.png + og-image-1200x630.png em base.html/producto.html/guias/article.html; deleta 10 binários legacy (android-chrome/apple-touch/favicon sem -especias, og-image, logo-branco, spices-header desktop+mobile) | templates/, static/ |
| 2026-05-11 21:52 | Claude | write | adiciona scrub defensivo em init_db() (MAINTENANCE_MODE): escaneia Product.name/description/origin, Category.name e SiteSetting.value, substitui variantes "Grãos S.A./SA/Paraguay/graos sa" → "Especias del Paraguay" no DB de produção no boot. Idempotente. Testado localmente. | app.py |
